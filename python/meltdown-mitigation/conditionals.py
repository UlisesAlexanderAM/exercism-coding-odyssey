...
def is_criticality_balanced(temperature: int, neutrons_emitted: int) -> bool:
    """Verify criticality is balanced.p
...
def fail_safe(temperature: int, neutrons_produced_per_second: int, threshold: int):
    ...
    criticality: int = temperature * neutrons_produced_per_second

    if criticality < (0.9 * threshold):
        return "LOW"
    if (0.1 * threshold) < criticality < (1.1 * threshold):
        return "NORMAL"
    return "DANGER"
